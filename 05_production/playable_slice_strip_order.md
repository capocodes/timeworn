...
**Implementation baseline:** Unreal work targets the **Lyra**-based **Timeworn** project ([Tech Baseline](tech_baseline.md)) — use Lyra's **multiplayer / UI / weapon** scaffolding where it accelerates the strips; **replace** sample content with **Timeworn** identity over time.

**Core loop:** This slice is the **in-engine spine** of the **raid session loop** in the [Core Session Loop](../01_vision/core_extraction_loop.md), scoped first to **tier 1 (survive / low loot)** in the [Game Loop](../01_vision/game_loop.md). Every strip should **trace** to stash/loadout → party/join → raid → **train extract** (**Extract Train 1** in target tier language) and stay forward-compatible with **round review** and later tiers (**town, build, defenses, Exodus**). **Meta** (base, quests, seasons, **vendors/crafting/skills**) lives under [Meta Loop](../01_vision/meta_loop_intent.md) plus [Vendors](../04_systems/vendors.md), [Crafting](../04_systems/crafting.md), [Skills](../04_systems/skills.md) — **out of slice v1** except where it informs **tone or UI copy**. **Realm vs raid instance** (friends/solo): [Player ↔ Server Cardinality](../04_systems/player_server_cardinality.md).
...
| 4 | **Single shootable gun** | **One** weapon wired end-to-end with **strong handling** (input → fire → feedback). **Slice v1:** **single-shot musket** — [Single-Shot Musket](../04_systems/weapon_single_shot_musket.md) (*Arc Raiders* **Ferro** as feel reference). |
...
*Revise this file when strip order or definitions change; note date in the [Decision Log](decisions.md) if direction shifts.*
