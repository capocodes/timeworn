...
- **Hosting:** **Cloud/realm** vs **locally hosted base** (listen/P2P) vs **hybrid** — see [Player ↔ Server Cardinality](../04_systems/player_server_cardinality.md) § *Locally hosted base*.
...
- **Character ↔ realm** — "**One character per server**" vs **global account**; **friends** (same realm, party queue) vs **solo** (matchmaking) — **working patterns** in [Player ↔ Server Cardinality](../04_systems/player_server_cardinality.md).
...
- **Why MUST extract pressure exist** (timer / failure state) — narrative spec lives with [Core Session Loop](core_extraction_loop.md) until broken out to [Narrative](../02_narrative/_index.md).

---

*When you lock numbers, add a row to the [Decision Log](../05_production/decisions.md) and trim this file.*
