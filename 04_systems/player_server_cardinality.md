...
**Status:** **Design exploration** — pick a model before hard-coding persistence and party flow. Linked from [Scale & Unresolved](../01_vision/scale_and_unresolved.md).
...
4. **Neighborhood shards + cross-instance raids** *(under consideration)*  
   ...
   - **Scheduled weekly hub reset** ([Meta Loop](../01_vision/meta_loop_intent.md)) — if the **neighborhood's buildable space** **wipes every week**, **sunset** becomes less about **orphaned structures** and more about **player assignment** + **what persists off-hub**. Align **merge** windows with **reset** tick when possible.  
...
| **Shared bases** ([Meta Loop](../01_vision/meta_loop_intent.md)) | **Who hosts** the save? **Merge conflicts** if two friends both edit layout offline — need **one writer** or **cloud authoritative** layout. |
...
*Overlaps* [Meta Loop](../01_vision/meta_loop_intent.md) **shared bases** — co-owners may need **shared migration** rules.
...
- [ ] **Raid instance:** max players / teams on map ([Scale & Unresolved](../01_vision/scale_and_unresolved.md))?
...
- [ ] **Weekly hub reset** — full wipe vs **partial**; what persists ([Meta Loop](../01_vision/meta_loop_intent.md))?
- [ ] **Slice v1:** ignore realm split; use **one dev realm** + **2P party** only.

---

*Related:* [Core Session Loop](../01_vision/core_extraction_loop.md), [Scale & Unresolved](../01_vision/scale_and_unresolved.md), [Vendors](vendors.md), [Skills](skills.md).
