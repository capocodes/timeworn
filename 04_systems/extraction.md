# Extraction *(stub)*

**Purpose:** The **extract mechanic** — timed train exits that close runs. Extraction is a **high-stakes beat**, not the genre label ([Game Design Document](../GAME_DESIGN_DOCUMENT.md)).

## Intent

- **Extract Train 1–3:** One per tier; escalating stakes/difficulty. See [Game Loop](../01_vision/game_loop.md).
- **Final extract:** After **Exodus** completion — the session climax.
- **v1 fantasy:** **Train departure** — approach station, board within window, survive until departure.
- **Failure:** Miss the train or die = failed extract; loot rules per [Pawn Loot](pawn_loot.md) / [Vault & Loadout](vault_and_loadout.md).
- **Slice v1:** Strip **#8** ([Playable Slice](../05_production/playable_slice_strip_order.md)) — exit condition **sold as a moment** (timing, feedback, clarity).

## Open

- [ ] Train schedule (fixed, dynamic, player-triggered).
- [ ] Boarding rules (proximity, capacity, contested boarding under fire).
- [ ] Extraction variants beyond train (mine, helicopter, etc. — post-v1).
- [ ] How [Gameplay Manager](gameplay_manager.md) triggers and tracks extract state.
- [ ] Multiplayer sync — all party members must board? Partial extract?
