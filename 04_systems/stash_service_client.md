...
**Purpose:** Client-side interface to the backend [Stash Service](service_stash.md). Reads/writes **vault** contents, validates loadout selection, and syncs inventory state across sessions.

## Intent

- **Pre-raid:** Pull vault contents → present in [Lobby](lobby.md) for loadout selection.
